  ...
  resolvePlugins.push({
    apply(resolver) {
      ...
      resolver.resolve = function (context, path, request, resolveContext, callback) {
        const self = this;
        resolve.call(self, context, path, request, resolveContext, function (err, path, result) {
          if (result) return callback(null, path, result);
          ...
          if (request.endsWith('.js') && context.issuer && (context.issuer.endsWith('.ts') || context.issuer.endsWith('.tsx')))
            return resolve.call(self, context, path, request.slice(0, -3), resolveContext, function (err, path, result) {
              if (result) return callback(null, path, result);
              if (err && !err.message.startsWith('Can\'t resolve'))
                return callback(err);
